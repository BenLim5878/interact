...
if ($user_id != $postUserID) {
    ...
    while ($userNameRow = $userName->fetch_assoc()) {
        ...
        mysqli_query($conn, "INSERT INTO notification (usersId,type, message,status,link) VALUES ($postUserID,'comment','$notificationContent','unread','/interact/include/post/showPost.php?id=$content->id')");
        echo $conn->error;
    }
}
